...
@testset "hard heap limit" begin
    # Allocating a 200MB array should abort the process, given the hard heap limit is 100MB
    cmd = `$(Base.julia_cmd()) --hard-heap-limit=100M -E "a = Array{UInt8}(undef, 200*1024*1024); GC.gc()"`
    @test !success(cmd)
end
end
...
@testset "--hard-heap-limit" begin
    ...
    one_terabyte_strs_and_vals = [
        ("1000000000000", UInt64(1000000000000)), ("1e12", UInt64(1e12)), ("1TB", UInt64(1t)),
    ]
